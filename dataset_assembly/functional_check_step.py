...
OUTPUT_PARQUET = PROJECT_ROOT / "mammary_diseases_indicators" / "temporary_datasets" / "cf_agg.parquet"
# Column rename map (Italian -> English)
RENAME_MAP = {"idAnimale": "id", "razza": "breed",
              "giorno": "day", "mese": "month",
              "anno": "year", "LatteAlle24Ore": "milk",
              "ProteineAlle24Ore": "protein", "GrassoAlle24Ore": "fat",
              "LinearScore": "scs", "DataControlloFunzionaleLatte": "cf_date"}
...
def fix_breed_conflicts(df: pd.DataFrame) -> pd.DataFrame:
    ...
    mode_map = (df.groupby("idAnimale", observed=True)["razza"].agg(lambda x: Counter(x).most_common(1)[0][0]))
...
def cf_main() -> None:
    ...
    # Load & base filtering: drop large/irrelevant columns early to save memory
    cf = pd.read_csv(RAW_CF_CSV, low_memory=False, usecols=lambda c: c not in {"idMisuraPrimaria", "siglaProvincia"}, dtype={"codiceRazzaAIA": "string", "codiceSpecieAIA": "string"})
    ...
    agg_dict = {"LinearScore": "first", "LatteAlle24Ore": "first",
    ...
    OUTPUT_IDS_PARQUET = PROJECT_ROOT / "mammary_diseases_indicators" / "temporary_datasets" / "cf_ids.parquet"
